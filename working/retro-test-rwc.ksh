...
# get archieved met files 
 if [ ! -s $COMOUT/aqm.$cycle.metcro3d.ncf ]; then  
  cd $COMIN
  if [ $PDY -le 20190715 ]; then
   hsi get /NAGAPE/arl/5year/Patrick.C.Campbell/NACC_FV3GFS16_runs/base_nacc_cmaq531_nofire/GFSv16.NACCv100.$PDY$cyc.output_met.tar
   tar xvf GFSv16.NACCv100.$PDY$cyc.output_met.tar
  else
   hpsstar get /NAGAPE/arl/5year/Patrick.C.Campbell/NACC_FV3GFS16_runs/base_nacc_cmaq531_nofire/GFSv16.NACCv100.$PDY$cyc.output_met.tar *
  fi
 fi
# if no archieve exists, run NACC 
 if [ ! -s $COMOUT/aqm.$cycle.metcro3d.ncf ]; then
...
if [ ! -s $COMOUT/aqm_conus_geos_fv3chem_aero_${PDY}_35L.ncf ] && \
 ...
 cd $COMIN
 if hsi get /NCEPDEV/emc-naqfc/5year/Youhua.Tang/cmaq531-test/aqm.$PDY/aqm_conus_geos_fv3chem_aero_${PDY}_35L.ncf 
  then
...
cd $COMOUT
scp -p aqm.$cycle.aconc_sfc.ncf youhuat@aaqest.arl.noaa.gov:/data/aqf3/youhuat/cmaq531-rwc/aqm.$PDY.$cycle.aconc_sfc.ncf &
scp -p aqm.$cycle.rj_1.ncf youhuat@aaqest.arl.noaa.gov:/data/aqf3/youhuat/cmaq531-rwc/aqm.$PDY.$cycle.rj_1.ncf &

cat>hsi-select.ksh<<EOF
#!/bin/ksh -x
hsi<<!
mkdir /5year/NCEPDEV/emc-naqfc/Youhua.Tang/cmaq531-test/rwc.$PDY
cd /5year/NCEPDEV/emc-naqfc/Youhua.Tang/cmaq531-test/rwc.$PDY
